window.addEventListener("load", function () {
  const apiUrl = "https://eonet.gsfc.nasa.gov/api/v3/categories/wildfires";
  let map;

  fetch(apiUrl)
    .then(handleResponse)
    .then(displayMapWithWildfires)
    .catch(handleError);
  ...
  dateSelector.addEventListener("change", (e) => {
    let result = wildfiresEvents.filter((data) => {
      return (
  ...
  function displayMapWithWildfires(data) {
    ...
    if (data && data.events) {
      wildfiresEvents = data.events.filter(
        (event) => event.categories[0].title === "Wildfires"
      );

      wildfiresEvents.forEach((event) => {
