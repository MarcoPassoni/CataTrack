window.addEventListener("load", function () {
  const apiUrl = "https://eonet.gsfc.nasa.gov/api/v3/categories/volcanoes";
  let map;

  fetch(apiUrl)
    .then(handleResponse)
    .then(displayMapWithVolcanoes)
    .catch(handleError);
  ...
  dateSelector.addEventListener("change", (e) => {
    let result = volcanoesEvents.filter((data) => {
      return (
  ...
  function displayMapWithVolcanoes(data) {
    ...
    if (data && data.events) {
      volcanoesEvents = data.events.filter(
        (event) => event.categories[0].title === "Volcanoes"
      );

      volcanoesEvents.forEach((event) => {
